    ...
    await prisma.$transaction([
      prisma.auditFinding.createMany({
        data: analysis.findings.map((f) => ({
          ...
          potentialSavingsCents: f.potentialSavingsCents != null ? BigInt(Math.round(f.potentialSavingsCents)) : null,
      ...
      prisma.discoveredAgent.createMany({
        data: analysis.discoveredAgents.map((d) => ({
          ...
          totalCostCents:   BigInt(Math.round(d.totalCostCents)),
          totalTokensIn:    BigInt(Math.round(d.totalTokensIn)),
          totalTokensOut:   BigInt(Math.round(d.totalTokensOut)),
      ...
      prisma.audit.update({
        ...
        data: {
          ...
          totalMonthlySpendCents: BigInt(Math.round(analysis.totalMonthlySpendCents)),
          estimatedWasteCents:    BigInt(Math.round(analysis.estimatedWasteCents)),
          efficiencyScore:        analysis.efficiencyScore,
          totalApiCalls:          analysis.totalApiCalls,
          totalTokensIn:          BigInt(Math.round(analysis.totalTokensIn)),
